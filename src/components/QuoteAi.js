...
const QuoteAi = () => {
  ...
  const hasStartedRef = useRef(false); // Prevents multiple triggers
  const prevQuoteRef = useRef(""); // Prevents showing the same quote twice in a row


  useEffect(() => {
    const handleIntersection = ([entry]) => {
      if (entry.isIntersecting && !hasStartedRef.current) {
        hasStartedRef.current = true; // Prevent multiple triggers
        timeoutRef.current = setTimeout(() => {
          setThinking(2); // Stop "Thinking..." after 3 seconds
          setQuote(quotes[Math.floor(Math.random() * quotes.length)]); // Pick a random quote
        }, 3000);
      }
    };

    const observer = new IntersectionObserver(handleIntersection, {
      threshold: 0.6, // Trigger when at least 60% of the component is visible
    });
  ...
  const generateNewQuote = () => {
    ...
    timeoutRef.current = setTimeout(() => {
        ...
        } while (newQuote === prevQuoteRef.current); // Ensure a new quote is selected

        prevQuoteRef.current = newQuote;
        setQuote(""); // Reset quote before setting new one

        setTimeout(() => {
            setQuote(newQuote); // Ensure state updates before animation starts
        }, 50);
    }, 3000);
};
